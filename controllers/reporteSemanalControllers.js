...
const reporteCalcinacionGetFecha = async (req = request, res = response) => {
    ...
    try {
       // const calcinacion = await Calcinacion.findOne({ _id: "6294e0cede9c7bcc74197004" });
        const calcinacion = await Calcinacion.findOne({ _id: id });
		//console.log(calcinacion);
		const totalC = await Temperatura_Horno.countDocuments( { 
			$and: [ 
				{ fecha: { $gte: calcinacion.fecha_inicio } }, 
				{ fecha: { $lte: calcinacion.fecha_fin } },
			] 
		} )
		const totalAb = await Temperatura_Horno.countDocuments( { 
			$and: [ 
				{ fecha: { $gte: calcinacion.fecha_inicio } }, 
				{ fecha: { $lte: calcinacion.fecha_fin } },
				{ alerta: {$eq: 'Baja'}}
		...
		const totalAad = await Temperatura_Horno.countDocuments( { 
			$and: [ 
				{ fecha: { $gte: calcinacion.fecha_inicio } }, 
				{ fecha: { $lte: calcinacion.fecha_fin } },
				{ alerta: {$eq: 'Adecuada'}}
		...
		const totalAal = await Temperatura_Horno.countDocuments( { 
			$and: [ 
				{ fecha: { $gte: calcinacion.fecha_inicio } }, 
				{ fecha: { $lte: calcinacion.fecha_fin } },
				{ alerta: {$eq: 'Alta'}}
		...
		const totalAp = await Temperatura_Horno.countDocuments( { 
			$and: [ 
				{ fecha: { $gte: calcinacion.fecha_inicio } }, 
				{ fecha: { $lte: calcinacion.fecha_fin } },
				{ alerta: {$eq: 'Peligro'}}
	...
	//	console.log('Baja',totalAb);
        ...
        res.render("reporteSemanalHorno.ejs", {
			calcinacion,
			totalC,
			totalAb,
			totalAad,
